 ...
 ;; send data
	      ...
	      out  dx , al
            ...
            cmp ah , 80
                  jne mnna
                  jmp Down      
mnna:       cmp ah, 77  ; right arrow
                  je right 
            cmp ah , 75 ; left arrow
                  je left
            cmp ah, 02EH ; OF C         ;;;;;;f3 --> scan code (03DH)
                  je Inline
            ...
            ret
      Inline:
            call inline_chat_mode_send          ;; proc of inline chat
      right:
            cmp playernum , 1
            jne D
            mov ax ,car_one_mode 
...
mnna2:            cmp ah, 77  ; right arrow
            ...
            cmp ah, 3eh ; scan code of f4 To Exit Game 
                  je temp_Label_game_ended122                
            ret
            temp_Label_game_ended122:
            mov gameend , 1    
            ret

      Inline22:
           call inline_chat_mode_Recieve
      right22:
            cmp playernum , 2
            jne D22
            mov ax ,car_one_mode 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   mainloop1: 
      ...
      changeee: call changeMode
      mov al , 1
      cmp gameend , al
      je temp_Label_game_ended2
      jmp receive
...
;--------------------- Track , obstcales, powerUps proc edures ----------------------
Draw_Track proc  near 
 ...
 Fill_background backgroundColor  ; calling the macro responsible for fill the backg with specific color
      cmp choose_level_value , 1
            je starttrack1
      cmp choose_level_value , 2
            jne fooooo
            jmp starttrack2
fooooo:cmp choose_level_value , 3
            ;je starttrack3
...
;      call Dotted_lineY
    ...
    ;--------------track2--------------
starttrack2:
mov car_two_old_x,100
mov car_two_old_y,100
mov car_two_x,100
mov car_two_y,100

mov curr_car_one_y,120
mov car_one_old_x, 100
mov car_one_old_y,120
mov curr_car_one_x,100

    set_indices 5,40,310,95,07        ;from (5,55) to (55,195) width 50 height 140
    call Draw_rect 
    set_indices 5,100,310,150,07    
    call Draw_rect 
    set_indices 5,95,55,100,07
    call Draw_rect
    set_indices 260,95,310,100,07
    call Draw_rect
    ;-------------Final Line--------------
    set_indices 100,100,103,110,00 
    call Final_lineBlack
    set_indices 103,100,106,110,0fh 
    call Final_lineBlack
    set_indices 106,100,109,110,00 
    call Final_lineBlack
    set_indices 100,110,103,120,0fh 
    call Final_lineBlack
    set_indices 103,110,106,120,00 
    call Final_lineBlack
    set_indices 106,110,109,120,0fh 
    call Final_lineBlack
    set_indices 100,120,103,130,00 
    call Final_lineBlack
    set_indices 103,120,106,130,0fh 
    call Final_lineBlack
    set_indices 106,120,109,130,00 
    call Final_lineBlack
    set_indices 100,130,103,140,0fh 
    call Final_lineBlack
    set_indices 103,130,106,140,00 
    call Final_lineBlack
    set_indices 106,130,109,140,0fh 
    call Final_lineBlack
    set_indices 100,140,103,150,00 
    call Final_lineBlack
    set_indices 103,140,106,150,0fh
    call Final_lineBlack
    set_indices 106,140,109,150,00 
    call Final_lineBlack
    jmp Ending_track
...
;    showMes equalSign
   ...
   setCursor 20,70,0    ; exit Game Mes 
   showMes endGame
;---------------------------------------
...
Draw_rect proc  near                            ; draw line Up given initial x , y position and x,y finals
    mov ah,0ch 
...
ret
Draw_rect endp
Final_lineBlack proc  near                      ; proc to draw the final line 
...
Dotted_line proc  near                 ; proc to draw the final line 
                     ; will draw the final line starting drom x=100 till 103
     	            ; strating from y=145 till y=195  --> the width of the track   
...
Dotted_lineY proc  near                 ; proc to draw the final line 
                     ; will draw the final line starting drom x=100 till 103
     	            ; strating from y=145 till y=195  --> the width of the track   
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Draw_obstacle proc  near  ;obstacles are drawn in red
...
CreatePowers ENDP
;;;;;;;;;;;;;;;;;;;;;;
draw_powers proc  near  ;obstacles are drawn in red
...
fill_power_up_buffers proc  near
...
setCursor 4,2, 0; for player num 1
mov ah,2
mov dl,Score_CarOne
int 21h
...
setCursor 4,8,0              ; for player num 2
mov ah,2
mov dl,Score_CarTwo
int 21h
call erase_power_ups
...
fill_obstacle_buffers proc  near
...
setCursor 5,2,0           ; for player num 1
mov ah,2
mov dl,Score_CarOne
int 21h
...
call set_score                     ; the min score must be zero always
mov Score_CarTwo,al
setCursor 5,8,0                  ; for player num 2
mov ah,2
mov dl,Score_CarTwo
int 21h
call erase_obstacles
...
who_win endp
;------------------------------------------------
inline_chat_mode_send proc near
;;;;;;;;;;;;;;;;;;;;;;;; Config ;;;;;;;;;;;;;;;;;;;;;;
mov dx,3fbh 			; Line Control Register
mov al,10000000b		;Set Divisor Latch Access Bit
out dx,al
...
ret
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
inline_chat_mode_send endp
;------------------------------------------------
inline_chat_mode_Recieve proc near
